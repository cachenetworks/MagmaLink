...
import lavalink.server.video.VideoSessionManager
...
class RequestAuthorizationFilter(
    ...
    override fun preHandle(request: HttpServletRequest, response: HttpServletResponse, handler: Any): Boolean {
        return when {
            ...
            // Video load responses include a per-session bearer token so
            // native HLS/media players can fetch segments without custom
            // Authorization-header support.
            videoSessions.isAccessTokenValid(request.servletPath, request.getParameter("token")) -> true
