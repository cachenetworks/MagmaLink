...
## MagmaLink unreleased

* Cloned the Lavalink 4.2.2 audio/server base while retaining the Lavalink v4
  REST and websocket compatibility surface.
* Added an authenticated `/magma/v1/videos` API for video resolution, HLS
  packaging, progressive range streaming, and session cleanup.
* Added direct HTTP sources, optional external resolver support, and an
  optional yt-dlp adapter.
* Added URL safety checks, per-session bearer URLs, cache cleanup, and FFmpeg
  Docker support.
